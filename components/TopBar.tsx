...
interface TopBarGlobalSettings {
...
const TopBar: React.FC = () => {
  const settings = useGlobalSettings() as TopBarGlobalSettings;
  const [isOpen, setIsOpen] = useState<boolean>(false);

  useEffect(() => {
    if (settings.godzinyOtwarcia) {
      setIsOpen(isOpenNow(settings.godzinyOtwarcia));
    }
          ...
          <div className="flex items-center gap-4">
            {settings.facebook && (
              <a
                ...
                className="text-emerald-100/60 hover:text-white hover:scale-110 transition-all duration-300"
                aria-label="Facebook"
              >
